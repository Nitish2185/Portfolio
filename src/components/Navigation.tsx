...
import { Mail, Download, X, Github, Linkedin, Twitter, BadgeCheck, Briefcase } from 'lucide-react';
import { motion, useScroll, useTransform, AnimatePresence } from 'framer-motion';
import { Link, useLocation, useNavigate } from 'react-router-dom';
import resumeFile from '../assets/Nitish_Singh_8423599961.pdf';
  ...
  >
    ...
    >
      <span className="font-serif text-lg md:text-xl font-bold text-black tracking-tighter">
        <span className="hidden sm:inline">Nitish Narayan Singh</span>
...
export const Navigation = () => {
  ...
  const menuItems = [
    ...
    { label: 'Credentials', id: '/credentials', type: 'link', icon: <BadgeCheck className="w-4 h-4" /> },
  ];

  const contactLinks = [
    { icon: <Mail className="w-4 h-4" />, label: 'Email', href: 'mailto:nns.nitish2185@gmail.com' },
    { icon: <Github className="w-4 h-4" />, label: 'GitHub', href: 'https://github.com/nitish2185' },
    { icon: <Linkedin className="w-4 h-4" />, label: 'LinkedIn', href: 'https://linkedin.com/in/Nitish2185' },
    { icon: <Twitter className="w-4 h-4" />, label: 'Twitter', href: 'https://x.com/nitish2185' },
  ];
